...
function classify(tabId, result) {
  ...
  for(var key in result) {
    if(result[key] == "1") maliciousCount++;
    else if(result[key] == "0") suspiciousCount++;
    else legitimateCount++;
  }
  legitimatePercents[tabId] = legitimateCount / (maliciousCount+suspiciousCount+legitimateCount) * 100;
